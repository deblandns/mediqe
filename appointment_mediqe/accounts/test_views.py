...
class UserProfileViewTests(APITestCase):
    ...
    def setUp(self):
        self.list_url = reverse('profiles-list') # 'profiles' is the basename from urls.py
        # Create a pre-existing user and profile for updates/retrieval
        user = User.objects.create_user(email='existing@user.com', password='oldpass', role='Doctor', gender='Male')
        self.profile_instance = UserProfile.objects.create(user=user, insurance_number='INS999', blood_types='O-', chronic_conditions='None')
        self.detail_url = reverse('profiles-detail', kwargs={'pk': self.profile_instance.pk})
